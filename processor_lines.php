...
switch( $_POST['action'] ){
    ...
    case 0:
		...
		$qres = $db->query('CREATE TABLE IF NOT EXISTS geners (id INTEGER PRIMARY KEY, g_name TEXT);') or die("Error in query: <span style='color:red;'>".$qres."</span>");
		$qres = $db->query('CREATE TABLE IF NOT EXISTS collect (id INTEGER PRIMARY KEY, c_name TEXT, visible NUMERIC);') or die("Error in query: <span style='color:red;'>".$qres."</span>");
		$qres = $db->query('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, user TEXT, passwd TEXT, grp TEXT, ip TEXT, last_coll INTEGER, last_filter INTEGER, last_step INTEGER, last_page INTEGER);');	
		$qres = $db->query('INSERT INTO users (user, passwd, grp) VALUES ("Admin", "'.$dpass.'", "owner");');			
	...
	case 96:
		...
		$db -> query('UPDATE users SET last_coll='.$post_data[4].', last_filter='.$post_data[1].', last_step='.$post_data[2].', last_page='.$post_data[3].' WHERE id="'.$post_data[5].'";');
			...
			*/
			...
			print('<table width="100%" border="0" >
				   <tr class="cat_head_dis" COLLID="'.$c_row['id'].'" collvis="'.$c_row['visible'].'">
					...
					<tr class="rowid_'.$c_row['id'].'-0" style="'.$coll_vis.'">
						<td class="tbTitle">№</td>
						<td class="tbTitle">Название:</td>
						<td class="tbTitle">Эпизодов:</td>
						<td class="tbTitle">Жанры:</td>
						<td class="tbTitle" colspan="2">Части:</td>
					</tr>
					');
			...
			while ( $m_row = $rowdata -> fetchArray(SQLITE3_BOTH) ){
					...
					$offset_nm++;
					print('<tr class="rowid_'.$c_row['id'].'-'.$m_row['id'].'" style="'.$coll_vis.'"> 
					
					<td valign="middle" align="center" class="rwCell rated_title rated_'.$m_row['rating'].'" style="font-size: 12pt; "> '.$offset_nm.' </td>
					
					<td valign="middle" align="left" class="rwCell rated_title rated_'.$m_row['rating'].' mrow_'.$c_row['id'].'-'.$m_row['id'].' name_row" style="font-size: 12pt; " img_url="'.$m_row['image_url'].'"> 					 
					...
					<td valign="middle" align="center"  class="rwCell rated_title rated_'.$m_row['rating'].'">
					'. $genNm .'</td>
		...
		?> <script>
			...
			$(function() {
					$(".rowEd").click(function(){
						...
						$('#img_url').val( $('.mrow_' + dbid).attr('img_url') );
					...
					$(".cat_head").click(function(){
						var vis = $(this).attr('collvis');
						
						$('[class|="rowid_' + $(this).attr('COLLID')+ '"]').toggle('slow');
						$('.facoll_'+ $(this).attr('COLLID')).toggleClass('fa-folder-open').toggleClass('fa-folder');
							...
							$.ajax({
							method: "POST", url: 'processor.php',
							data:{ 	action: '95', procdata: $(this).attr('COLLID') + '|' + vis 	},
							beforeSend: function(xhr){ 	console.log('обрабатываю запрос: 95'); 	}
							})
							.done(function(){ 	console.log('Успешно выполнен: 95'); })
							.fail(function(){	console.log('Ошибка при обработке запроса 95' ); 	});						
					...
					function(){
						...
						if ( img_url.indexOf('jpg')>=0 || img_url.indexOf('jpeg')>=0 || img_url.indexOf('png')>=0 || img_url.indexOf('gif')>=0 ){
	...
	case 97:
		...
		switch ( $post_data[0] ){
			case 5: 
				...
				print('<script> $("#'.$post_data[1].'").html( "" ); </script>');
			break;	
			case 7: 
				$db_read = 'geners'; 
		...
		while ( $c_row = $dbdata -> fetchArray(SQLITE3_NUM) ){
			print('<script> 
				$("#'.$post_data[1].'").append( "<option value='.$c_row[0].'> '.$c_row[1].' </option>"  );
				</script>');
		}
